(function($){
 var tabulator={
  ...
  _privateFunction:function(level){
	var widget=this;
	var $domElement=widget.element;
	var options=widget.options;
  ...
  _pieChart:function() {
	...
	*/
	...
	// Get the canvas element for the table
	//var $canvas=$('<canvas/>',
	//	{width:$(options.chartContainer).width(),
	//	height:$(options.chartContainer).height()}
	//	);
	
	var $canvas=$('<canvas width="'+$(options.chartContainer).width()+'" height="'+$(options.chartContainer).height()+'" ></canvas>');
	$(options.chartContainer).append($canvas);
	
	//canvas = document.getElementById(options.chartId);
	canvas = $canvas[0];
	//canvas="asd";
	...
	//For every td in the body of the table
	$('tbody td',$domElement).each( function() {
	  currentCell++;
	  if ( currentCell % 2 != 0 ) {
		currentRow++;
	...
	*/
	...
	$('td',$domElement).removeClass('highlight');
	var labelCell = $('td:eq(' + (slice*2) + ')',$domElement);
	var valueCell = $('td:eq(' + (slice*2+1) + ')',$domElement);
